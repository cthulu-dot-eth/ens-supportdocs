...
If you want to completely transfer a name to a wallet you'll need to set three records to the new wallet in a specific order because only the wallet set as `Controller` is allowed to change the `ETH Address`. In order to set everything to the new wallet easily from the wallet that currently owns the ENS name it's follow this specific order:
